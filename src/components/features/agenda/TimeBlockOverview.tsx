...
export function TimeBlockOverview() {
  ...
  const { dateInfo, totalPlanned, totalSpent, categories, nextBlock } = useMemo(() => {
    ...
    for (const block of sorted) {
      ...
      const category = categoryMap.get(label) || { planned: 0, spent: 0 };
      category.planned += duration;

      if (endMinutes <= currentMinutes) {
        totalSpent += duration;
        category.spent += duration;
      } else if (startMinutes < currentMinutes && endMinutes > currentMinutes) {
        const elapsed = currentMinutes - startMinutes;
        totalSpent += elapsed;
        category.spent += elapsed;
      }

      categoryMap.set(label, category);

      if (startMinutes > currentMinutes && !nextBlock) {
        nextBlock = { title: block.title, start: block.start };
      }
    }

    return { dateInfo, totalPlanned, totalSpent, categories: categoryMap, nextBlock };
  }, [timeBlocks]);
    ...
    <Card>
      ...
      {categories.size > 0 && (
        <div className="flex flex-col gap-2 pt-1">
          {Array.from(categories.entries()).map(([label, category]) => (
            <div key={label} className="flex items-center gap-3">
              ...
              <div className="flex-1">
                <Progress value={category.spent} max={category.planned || 1} />
              </div>
              <span className="text-xs text-text-secondary w-20 text-right tabular-nums">
                {(category.spent / 60).toFixed(1)}h / {(category.planned / 60).toFixed(1)}h
              </span>
            </div>
          ))}
        </div>
